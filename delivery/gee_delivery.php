...
if(isset($_POST['submit']))
{
    ...
    if($qty!="") {
        if(isset($c_id))
        $sql = "update gee_delivery set gd_name=:name , gd_phone=:phone , gd_address=:address , gd_qty=:qty , gee_add_by=:add_by, gd_remark=:remark where gd_id=:id";
        else
            $sql = "insert into gee_delivery(gd_name,gd_phone,gd_address,gd_qty,gee_add_by,gd_remark,gd_date) value (:name,:phone,:address,:qty,:add_by,:remark,:today)";
        ...
        $error="Qty should not be empty, but give has 0 , if they didnt take Gee.";
    }
}
?>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    ...
                    <h2 class="page-title">Gee deliery</h2>
                    ...
                    <div class="panel panel-default" style="overflow-x:auto;">
                        ...
                        <div class="panel-body">
                            ...
                            <table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
                                ...
                                $today=date("Y-m-d");
                                $name=$_SESSION['name'];
                                $sql = "SELECT * from gee_delivery where gd_date=:today AND gee_add_by=:name";
                                $query = $link -> prepare($sql);
                                $query->bindParam(':today', $today, PDO::PARAM_STR);
                                $query->bindParam(':name', $name, PDO::PARAM_STR);
                                $query->execute();
                                ...
                                if($query->rowCount() > 0)
                                {
                                    foreach($results as $result)
                                    {
                                        ?>
                                            <tr>
                                                <form method="post" action="index.php?id=<?php echo $result->gd_id;?>&num=<?php echo $cnt;?>" >
                                                <td><?php echo $cnt; ?></td>
                                                <td><input type="text" name="name<?php echo $cnt; ?>" id="name<?php echo $cnt; ?>" value="<?php echo $result->gd_name;?>"></td>
                                                <td><input type="text" name="phone<?php echo $cnt; ?>" id="phone<?php echo $cnt; ?>" value="<?php echo $result->gd_phone;?>"></td>
                                                <td><input type="text" name="address<?php echo $cnt; ?>" id="address<?php echo $cnt; ?>" value="<?php echo $result->gd_address;?>"></td>
                                                    <td><input type="text" name="qty<?php echo $cnt; ?>" id="qty<?php echo $cnt; ?>" size="3" value="<?php echo $result->gd_qty;?>"></td>
                                                    <td><input type="text" name="remark<?php echo $cnt; ?>" id="remark<?php echo $cnt; ?>" value="<?php echo $result->gd_remark;?>"></td>
                                                    <td><input type="submit" name="submit" value="submit"></td>
                                                </form>
                                            </tr>
                                            <?php $cnt=$cnt+1; }} ?>
                                <tr>
                                    <form method="post" action="index.php?num=<?php echo $cnt;?>" >
                                        <td><?php echo $cnt; ?></td>
