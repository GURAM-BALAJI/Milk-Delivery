...
<div class="table-responsive">
    ...
    <table id="dataTable" class="table table-striped">
        ...
        for ($i=$starting_date; $i<=$ending_date; $i+=86400) {
            $date=date("Y-m-d", $i);
            if($delivery!='0')
                $sql1 = "select * from gee_pick where DATE(gp_date)=:date AND gp_name=:delivery";
            else
                $sql1 = "select * from gee_pick where DATE(gp_date)=:date";
            ...
            if ($query1->rowCount() > 0) {
                foreach ($results1 as $result1){
                    ...
                    echo '<td>Gee</td>';
                    echo '<td>'.$date.'</td>';
                    $total+=$result1->gp_qty;
                    echo '<td>' . $result1->gp_qty . '</td>';
                    echo '<td>'.$result1->gp_name.'</td>';
                    echo "</tr>";
                }
            }
    ...
    <?php }else{
        header("location: ../gee_pick_up_recored.php");
    } ?>
</div>
