...
<div class="table-responsive">
    ...
    <table id="dataTable" class="table table-striped">
        ...
        for ($i=$starting_date; $i<=$ending_date; $i+=86400) {
            $date=date("Y-m-d", $i);
            if($delivery!='0')
                $sql1 = "select * from panner_pick where DATE(pp_date)=:date AND pp_name=:delivery";
            else
                $sql1 = "select * from panner_pick where DATE(pp_date)=:date";
            ...
            if ($query1->rowCount() > 0) {
                foreach ($results1 as $result1){
                    ...
                    echo '<td>Panner</td>';
                    echo '<td>'.$date.'</td>';
                    $total+=$result1->pp_qty;
                    echo '<td>' . $result1->pp_qty . '</td>';
                    echo '<td>'.$result1->pp_name.'</td>';
                    echo "</tr>";
                }
            }
    ...
    <?php }else{
        header("location: ../panner_pick_up_recored.php");
    } ?>
</div>
