...
<div class="table-responsive">
    ...
    <table id="dataTable" class="table table-striped">
            ...
            for ($i=$starting_date; $i<=$ending_date; $i+=86400) {
                $date=date("Y-m-d", $i);
                if($delivery!='0')
                $sql1 = "select * from gee_delivery where DATE(gd_date)=:date AND gee_add_by=:delivery";
                else
                    $sql1 = "select * from gee_delivery where DATE(gd_date)=:date";
                ...
                if ($query1->rowCount() > 0) {
                    foreach ($results1 as $result1){
                        echo "<tr>";
                        echo '<td>'.$result1->gd_id.'</td>';
                        echo '<td>Gee</td>';
                        echo '<td>'.$date.'</td>';
                        echo '<td>'.$result1->gd_name.'</td>';
                        $total+=$result1->gd_qty;
                        if($result1->gd_qty!=0)
                            echo '<td>' . $result1->gd_qty . '</td>';
                        else
                            echo "<td style='color:#ff0000;'>Not-Taken</td>";
                        echo '<td>'.$result1->gd_phone.'</td>';
                        echo '<td>'.$result1->gd_address.'</td>';
                        if($delivery!=0)
                        echo '<td>'.$delivery.'</td>';
                        else
                            echo '<td>'.$result1->gee_add_by.'</td>';
                        echo '<td>'.$result1->gd_remark.'</td>';
                        echo "</tr>";
                    }
                }
    ...
    <?php }else{
        header("location: ../gee_recored.php");
    } ?>
</div>
